...
- **What are the ingredients we need?**
  ...
  - test statistic that has different values that can express relevant information about the characteristic we are testing for

    –> test statistic = a random variables whose probability distribution is known under the null hypothesis

  - a parametrized distribution (parameters = degrees of freedom) over the value of the test statistics that tells us how common/rare the value of a test statistic is for the given parametersw (= numbers of data points + parameters) 

  - critical values for the different significance levels (0.1, 0.05, 0.01) that we extract from the known distribution

  - formulation of null + alternative hypothesis

- **How do you draw conclusions from a statistical test?**

  1. calculate the test statistic (usually with an existing implementation)

  2. look at the probability of that test statistic

     –> How rare/common is that value of the test statistic?

  3. If the value is very small (= smaller than the critical value provided by the test for a given significance threshold), that means that it is very unlikely that the data that was tested was generated by the model the null hypothesis assumes.

     –> the null hypothesis can be rejected, we assume the alternative hypothesis to be correct

  4. If the value is larger than the critical value, the test statistic does not have a rare/uncommon value and probably the data fits to the null hypothesis.
